...
test('v92 correction is bounded to low-confidence glossary evidence', () => {
...
test('v92 relaxed barge-in keeps 150ms continuity and self-voice guard', () => {
...
test('v92 server persists transcript provenance and barge-in latency', () => {
  for (const key of ['rawTranscript','correctedTranscript','correctionReason','bargeInTriggerMs']) assert.match(entry, new RegExp(key));
});


test('v92 rejects common subtitle outro hallucinations unconditionally', () => {
...
test('v92 ordinary short utterances are never deferred or timeout-dropped', () => {
...
test('v92 STT only pre-drops physically tiny or effectively silent captures', () => {
...
test('v92 rescues usable realtime transcript when Whisper returns an ignorable 422', () => {
...
test('v92 retries strong-signal empty Whisper transcript only once', () => {
...
test('v92 searches unknown entity explanation questions and contextual follow-ups', () => {
...
test('v92 D1 compact logs preserve voice correction and barge-in diagnostics', () => {
