...
const VOICE_REVISION = 'fast-grounded-v8';
const CASUAL_VOICE_MODEL = '@cf/meta/llama-3.2-3b-instruct';
const GROUNDED_VOICE_MODEL = '@cf/qwen/qwen3.8-27b';

const CASUAL_SYSTEM_PROMPT = `日本語の自然な会話相手として答える。電話会話なので1〜2文を基本に、短くテンポよく返す。雑談をPC操作の話にしない。知らない事実や現在情報は作らず、必要なら確認が必要だと短く言う。定型的な前置き、Markdown、URL読み上げは避ける。`;

const GROUNDED_SYSTEM_PROMPT = `あなたはTalkSysという日本語の音声アシスタントです。短く自然な電話会話として答えてください。
絶対ルール:
