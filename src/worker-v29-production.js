import workerV29, { TalkSysVoiceAgent, VOICE_REVISION_V29 } from './worker-v29.js';
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (request.method === 'GET' && url.pathname === '/api/search-smoke-v29-context') {
      return searchSmoke('神奈川県横浜市でおすすめのお店ある', CONTEXT_SMOKE_HISTORY);
    }
    if (request.method === 'GET' && url.pathname === '/api/search-smoke-v29-transit-followup') {
      return searchSmoke('どうですか', IN_FLIGHT_TRANSIT_HISTORY);
    }
    if (request.method === 'GET' && url.pathname === '/api/search-smoke-v29-pc') {
    ...
    if (request.method === 'GET' && url.pathname === '/voice-health') {
      ...
      return Response.json({
        ...
        architecture: 'grok-centered-browser-and-pstn',
        ...
        contextualSearchSmokeEndpoint: '/api/search-smoke-v29-context',
        transitFollowupSmokeEndpoint: '/api/search-smoke-v29-transit-followup',
      }, {
        headers: {
          'cache-control': 'no-store',
          'x-talksys-voice-revision': VOICE_REVISION_V29,
