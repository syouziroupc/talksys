...
export const VOICE_REVISION_V29 = 'cloudflare-agent-v29-grok-tts-phone';
...
function serveScript(source) {
  return new Response(source, {
    headers: {
      ...
      'x-talksys-voice-revision': VOICE_REVISION_V29,
    },
  });
}
...
function sessionAffinity(context) {
  ...
  return source ? `talksys-v29-${source}` : '';
}
...
export class TalkSysVoiceAgent extends TalkSysVoiceAgentV22 {
  tts = new GrokJapaneseTTSV29(this.env.AI);
  ...
  trackAssistant(iterable, context, tier, userText = '') {
    ...
    return (async function* () {
      ...
      try {
        send(connection, { type: 'model_route', tier, model: GROK_CONVERSATION_MODEL_V29, phase: 'selected' });
  ...
  mandatoryGroundedTurn(transcript, context, history) {
    ...
    return (async function* () {
      ...
      const pc = PC_TOPIC_RE.test(String(result.resolvedQuestion || transcript || ''));
      yield* streamGrokConversationV29(self.env.AI, evidenceMessages(history, transcript, result), {
        signal: context?.signal,
        maxTokens: pc ? 340 : 250,
        openTimeoutMs: pc ? 2100 : 1750,
        firstTokenTimeoutMs: pc ? 2500 : 2100,
        retryTimeoutMs: pc ? 3400 : 3000,
        streamIdleTimeoutMs: 2800,
        streamTotalTimeoutMs: pc ? 12000 : 9800,
  ...
  normalConversationTurn(transcript, context, history) {
    ...
    if (quick) return this.trackAssistant((async function* () { yield quick; })(), context, 'instant-local-v29', transcript);

    const pc = PC_TOPIC_RE.test(String(transcript || ''));
    return this.trackAssistant(
      streamGrokConversationV29(this.env.AI, nonGroundedMessages(history, transcript), {
        signal: context?.signal,
        maxTokens: pc ? 260 : 210,
        openTimeoutMs: pc ? 1900 : 1550,
        firstTokenTimeoutMs: pc ? 2250 : 1850,
        retryTimeoutMs: pc ? 3200 : 2800,
        streamIdleTimeoutMs: 2600,
        streamTotalTimeoutMs: pc ? 10500 : 8800,
        sessionAffinity: sessionAffinity(context),
      }),
      context,
      pc ? 'grok-pc-v29' : 'grok-live-v29',
      transcript,
    );
  }
}
  ...
  async fetch(request, env, ctx) {
    ...
    if (request.method === 'GET' && url.pathname === '/voice-health') {
      return Response.json({
        ok: true,
        voiceRevision: VOICE_REVISION_V29,
        architecture: 'whisper-grok-groktts-browser-plus-grok-phone',
        conversationOrchestrator: 'single-grok-bounded-v29',
        ...
        browserStt: '@cf/openai/whisper-large-v3-turbo',
        ttsPrimary: GROK_TTS_MODEL_V29,
        ttsFallback: null,
        grokTtsJapaneseVoice: 'ara',
