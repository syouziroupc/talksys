export const TALK_CLIENT_V34 = String.raw`(() => {
'use strict';
...
async function ask(text){
  ...
  log((j.search?'検索あり':'検索なし')+' / route='+(j.route||'?')+' / GLM '+lastGlmMs+'ms'+(lastSearchMs?' / 検索 '+lastSearchMs+'ms':''));if(j.search&&j.queries?.length)log('検索語: '+j.queries.join(' | '));
...
async function speak(text){
  ...
  try{
    ...
    const blob=await r.blob();lastTtsMs=Number(r.headers.get('x-talksys-tts-ms'))||Date.now()-t;log('TTS完了 '+lastTtsMs+'ms / '+blob.size+'B / model='+(r.headers.get('x-talksys-tts-model')||'?')+' / lang='+(r.headers.get('x-talksys-tts-lang')||'?'));
...
ttsTest.addEventListener('click',async()=>{if(playing)return;log('日本語TTSテスト開始');await speak('日本語音声の動作確認です。Windows 10とWindows 11も自然に読めれば正常です。');});
window.addEventListener('beforeunload',()=>{if(micOn)stopMic();});
diagUpdate(true);log('TalkSys v34 起動');
})();`;