...
function classifySafety(text){
  ...
  const sexual=/(セックス|性交|性行為|挿入|中出し|フェラ|クンニ|オナニー|自慰|ちんこ|ちんぽ|まんこ|膣|アナル|裸|エロ|性的|犯す|ハメる|やりたい)/u.test(v)||/(俺|僕|自分)のバナナ.*(?:入れ|挿れ|突っ込)/u.test(v);
  if(minor&&sexual)return 'sexual-minor';
  if(/(?:民族|人種|宗教|国民|外国人|女性|男性|障害者|同性愛者|移民|〇〇人|○○人).{0,20}(?:皆殺し|殺せ|絶滅|排除しろ|消せ)/u.test(v))return 'violent-hate';
  ...
  if(sexual&&/(したい|して|話して|言って|興奮|入れ|挿れ|突っ込|舐め|触り|犯す|ハメ)/u.test(v))return 'sexual-explicit';
  return '';
}
function safetyReply(kind){
  ...
  if(kind==='sexual-explicit')return '露骨な性的なやり取りには乗れません。恋愛、同意、避妊、性の悩みの相談なら普通に話せます。';
  return '';
}
...
export default {
  async fetch(request,env){
    ...
    if(request.method==='POST'&&url.pathname==='/api/plan'){
      ...
      if(safety||isSubjectiveChat(text))return json({ok:true,search:false,topic:'',resolvedQuestion:text,searchInstruction:'',ack:'',planner:safety?'safety-local-v41':'subjective-local-v41',plannerMs:0,jst:''});
      const r=await workerV40.fetch(request,env);const type=r.headers.get('content-type')||'';if(!type.includes('application/json'))return wrap(r);try{const data=await r.json();if(data?.ok)data.ack=data.search?searchAck(text,data):'';data.planner=data.planner||'v40-base-v41';return json(data,r.status);}catch{return wrap(r);}
    }
...
export const __test={classifySafety,safetyReply,searchAck,isSubjectiveChat};
