...
export async function collectResilientEvidenceV41(resolved,history=[],instruction='',options={}){
  const goal=clean(`${resolved} ${instruction}`,2200),phoneShopping=isPhoneShopping(goal);let base;
  if(phoneShopping){
    // 「スマホを乗り換える」を鉄道の乗換と誤認する旧検索器を通さない。
    base={resolvedQuestion:resolved,queries:[],sources:[]};
  }else{
...
export const __test={budgetToken,androidToken,compactCore,isPhoneShopping,searchResultRelevant,hasConcreteShoppingEvidence,buildRetryQueries,hostOf,stripHtml};
