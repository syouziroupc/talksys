...
const BAD_OS_INFERENCE_RE=/(?:発売|登場|年式).{0,30}(?:新し|最近).{0,55}(?:Android|OS|有力|おすすめ|安心)|(?:新し|最近).{0,35}(?:ため|ので).{0,45}(?:Android|OS|有力|おすすめ)/i;
const SELF_VERIFY_RE=/(?:購入前|商品ページ|通販サイト|販売店|店舗).{0,55}(?:確認して|確認いただ|チェックして|見てください|調べてください)/i;
...
async function glm(env,messages,max=440){const t=Date.now();const r=await env.AI.run(MODEL,{messages,stream:false,modalities:['text'],max_completion_tokens:max,temperature:0.1,reasoning_effort:'low'});const text=extractText(r);if(!text)throw new Error('empty model answer');return {text,ms:Date.now()-t};}
const GENERAL_PROMPT=`あなたはTalkSysの日本語電話相談AIです。Web検索を使わず、利用者自身が述べた事実と安定した一般原則だけで判断してください。修理か買い替えかなら、購入額、故障部位、使用期間、重要データの有無、現在使えるかを材料に、どちら寄りかを最初に示してください。未検索の修理料金、中古相場、保証、下取り額、特定店舗の下取り・修理受付など変動する事実は作らないでください。「それは大変でしたね」「まず見積もりを」だけで終わらせず、判断理由を2〜4文で自然に答えてください。`;
const SEARCH_PROMPT=`あなたはTalkSysの日本語電話相談AIです。取得済み検索根拠だけで意思決定に役立つ回答を作ってください。「申し訳ありません、確認できませんでした」「具体的な機種は案内できません」「自分で検索してください」のような検索失敗定型文で終わらせず、確認できた有用な事実を先に答えてください。利用者が言っていないAndroid番号、SIMフリー、保証付き等を必須条件に追加しないでください。買い替え理由がAndroidの古さなら、価格と同じ候補についてOS更新・サポート根拠が確認できたものだけをOS面で推奨してください。発売年・登場時期・機種の新しさだけからOSが新しい、有力、おすすめ、安心とは絶対に推測しません。verifiedCandidatesが空なら、価格候補は価格候補として紹介できますが、OS面の推奨確定はしません。最後を「購入前に商品ページで確認してください」「自分で調べてください」のような丸投げで終えません。根拠にない価格・在庫・修理代・保証・OSバージョンは作らないでください。3〜6文で、URLや内部検索回数は読み上げないでください。`;
function evidenceBlock(s){return (s?.sources||[]).slice(0,14).map((x,i)=>`[${i+1}] ${clean(x?.title,220)}\n${clean(x?.url,700)}\n${clean(x?.excerpt||x?.snippet||'',1800)}`).join('\n\n');}
function priceLabel(candidate){const t=clean(candidate?.priceSource?.text,1800),m=t.match(PRICE_RE);return m?`${candidate.model}（${m[0]}）`:candidate?.model||'';}
function groundedPartialAnswer(search){
  const verified=Array.isArray(search?.verifiedCandidates)?search.verifiedCandidates:[],priced=Array.isArray(search?.pricedCandidates)?search.pricedCandidates:[];
  if(verified.length){const labels=verified.slice(0,3).map(priceLabel).filter(Boolean);return `${labels.join('、')}は、販売価格と同じ機種の公式OS更新情報の両方を確認できた候補です。Androidの古さを理由に買い替えるなら、この確認済み候補を優先するのが妥当です。`;}
  if(priced.length){const labels=priced.slice(0,4).map(priceLabel).filter(Boolean);return `価格面では、${labels.join('、')}を候補として確認できています。ただし、Androidの古さが買い替え理由なので、同じ機種の公式OS更新・サポート根拠まで結び付いていないものをOS面で「おすすめ」とは断定しません。現時点では価格候補として扱い、OS面まで根拠が揃った候補だけを優先するのが妥当です。`;}
  return '条件を勝手に足さずに複数経路で探しましたが、今回取得できた根拠だけでは具体的な購入候補を責任を持って確定できません。代わりに、予算内というだけで古い機種を勧めず、販売価格とOS更新サポートの両方が同じ機種で確認できる候補を優先するのが妥当です。';
}
function badGroundedAnswer(text,search){if(GIVEUP_RE.test(text)||SELF_VERIFY_RE.test(text))return true;return Boolean(search?.androidFreshnessRequired)&&!(search?.verifiedCandidates||[]).length&&BAD_OS_INFERENCE_RE.test(text);}
...
async function searchTurn(body,plan,env){
  const started=Date.now(),hist=userHistory(body?.history),resolved=clean(plan?.resolvedQuestion,1800)||clean(body?.text,1800),instruction=clean(plan?.searchInstruction,3000);const t=Date.now();const s=await collectResilientEvidenceV42(resolved,hist,instruction);const searchMs=Date.now()-t;const verified=Array.isArray(s?.verifiedCandidates)?s.verifiedCandidates:[];
  const prompt=`相談: ${resolved}\n利用者が明示した検索条件: ${instruction}\nverifiedCandidates: ${JSON.stringify(verified)}\npricedCandidates: ${JSON.stringify((s?.pricedCandidates||[]).slice(0,8))}\nandroidFreshnessRequired: ${Boolean(s?.androidFreshnessRequired)}\nandroidRequirementEvidence: ${Boolean(s?.androidRequirementEvidence)}\n\n取得根拠:\n${evidenceBlock(s)||'(根拠なし)'}\n\n利用者の質問へ直接答えてください。`;
  let g=await glm(env,[{role:'system',content:SEARCH_PROMPT},...hist,{role:'user',content:prompt}],520),glmMs=g.ms;
  if(badGroundedAnswer(g.text,s)){const second=await glm(env,[{role:'system',content:SEARCH_PROMPT+' 前の草案には、検索失敗の丸投げ、発売年からのOS推測、または自己確認依頼が含まれました。それらを完全に削除し、確認済み事実と未確認部分を分けた完成回答だけを書いてください。'},...hist,{role:'user',content:prompt}],480);g=second;glmMs+=second.ms;}
  const answer=badGroundedAnswer(g.text,s)?groundedPartialAnswer(s):g.text;
  return {ok:true,answer,search:true,route:'resilient-search-v42',searchUseful:Boolean(s?.sources?.length),resolvedQuestion:s?.resolvedQuestion||resolved,queries:(s?.queries||[]).slice(0,24),sources:(s?.sources||[]).slice(0,14).map(x=>({title:clean(x?.title,220),url:clean(x?.url,700)})),searchPasses:Number(s?.searchPasses)||1,maxSearchPasses:Number(s?.maxSearchPasses)||5,concreteShoppingEvidence:Boolean(s?.concreteShoppingEvidence),androidFreshnessRequired:Boolean(s?.androidFreshnessRequired),androidRequirementEvidence:Boolean(s?.androidRequirementEvidence),verifiedCandidates:verified,pricedCandidates:(s?.pricedCandidates||[]).slice(0,8),sourceQuality:s?.sourceQuality||'',historyPolicy:s?.historyPolicy||'user-only',timings:{totalMs:Date.now()-started,searchMs,glmMs,plannerMs:Number(plan?.plannerMs)||0},model:MODEL,planner:'phone-shopping-local-v42',searchPlan:instruction,languageMode:'ja-only'};
}

export default {
  async fetch(request,env,ctx){
    const url=new URL(request.url);
    if(request.method==='POST'&&url.pathname==='/api/tts')return json({ok:false,error:'Server TTS is disabled. TalkSys uses browser ja-JP speech.'},410);
...
export const __test={explicitLookup,localDecision,searchPlan,generalPlan,flags,badGroundedAnswer,groundedPartialAnswer,BAD_OS_INFERENCE_RE,SELF_VERIFY_RE};
